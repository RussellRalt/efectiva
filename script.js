...
const backToTasksFromRewardBtn = document.getElementById('backToTasksFromRewardBtn');
...
// Regresar a la vista de carpetas
...
/* Recompensa */
backToTasksFromRewardBtn.addEventListener('click', () => {
...
function renderTasks() {
  ...
  folder.tasks.forEach((task) => {
    ...
    if (!folder.isDefaultRewards) {
      ...
      // Carpeta de recompensas: SOLO eliminar
      const deleteBtn = document.createElement('button');
      deleteBtn.textContent = '🗑';
      deleteBtn.addEventListener('click', (e) => {
        e.stopPropagation();
    ...
    // Contenedor de pasos (solo si NO es Recompensas)
    ...
    if (!folder.isDefaultRewards) {
      ...
      // Al presionar Enter, se crea el paso
...
function saveDataToLocalStorage() {
  localStorage.setItem('foldersDataV3', JSON.stringify(folders));
}
function loadDataFromLocalStorage() {
  const data = localStorage.getItem('foldersDataV3');
  if (data) {
